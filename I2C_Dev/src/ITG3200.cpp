...
void ITG3200::init(unsigned int address, uint8_t _SRateDiv, uint8_t _Range, uint8_t _filterBW, uint8_t _ClockSrc, bool _ITGReady, bool _INTRawDataReady) {
  ...
  if (ioctl(fI2C, I2C_SLAVE, _dev_address) < 0) {
...
void ITG3200::readGyro(float *_GyroXYZ){
  readGyro(&_GyroXYZ[0], &_GyroXYZ[1], &_GyroXYZ[2]);
}
...
void ITG3200::writemem(uint8_t _addr, uint8_t _val) {
  int err;
  err = i2c_smbus_write_byte_data(fI2C, _addr, _val); // see documentation at https://www.kernel.org/doc/Documentation/i2c/dev-interface
  if (err < 0) {
    err = errno; // currently this error code is just ignored
  }
}

void ITG3200::readmem(uint8_t _addr, uint8_t _nbytes, uint8_t * _buff) {
  /*Wire.beginTransmission(_dev_address); // start transmission to device 
  Wire.write(_addr); // sends register address to read from
  Wire.endTransmission(); // end transmission
  
  Wire.beginTransmission(_dev_address); // start transmission to device 
  Wire.requestFrom(_dev_address, _nuint8_ts);// send data n-uint8_ts read
  uint8_t i = 0; 
  ...
  err = i2c_smbus_read_i2c_block_data(fI2C, _addr, _nbytes, _buff);  // see documentation at https://www.kernel.org/doc/Documentation/i2c/smbus-protocol
  if (err < 0) {
    err = errno; // currently this error code is just ignored
  }
}
