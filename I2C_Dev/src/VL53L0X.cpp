...
void VL53L0X::setAddress(uint8_t new_addr)
{
  ...
  // Close and reopen port with new address
  close(fI2C);
  fI2C = open("/dev/i2c-0", O_RDWR);
  if (fI2C < 0) {
      // Could not open the file
      last_status = errno ;
      return;
  }
  if (ioctl(fI2C, I2C_SLAVE, address) < 0) {
...
{
    last_status = i2c_smbus_write_byte_data(fI2C, reg, value); // see documentation at https://www.kernel.org/doc/Documentation/i2c/dev-interface
    if (last_status < 0) {
        last_status = errno ;
        last_status = -1 ;
    }
...
void VL53L0X::writeReg16Bit(uint8_t reg, uint16_t value)
{
    ...
    last_status = i2c_smbus_write_i2c_block_data(fI2C, reg, 2, data);
    if (last_status < 0) {
        last_status = errno ;
        last_status = -1 ;
    }
...
void VL53L0X::writeReg32Bit(uint8_t reg, uint32_t value)
{
    ...
    last_status = i2c_smbus_write_i2c_block_data(fI2C, reg, 4, data);
    if (last_status < 0) {
        last_status = errno ;
        last_status = -1 ;
    }
...
{
    int toReturn;
    toReturn = i2c_smbus_read_byte_data(fI2C, reg) ;
    if (toReturn < 0) {
        last_status = errno;
        toReturn = 0;
    }
    return (uint8_t)toReturn;
}
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
    ...
    readData[0] = i2c_smbus_read_byte_data(fI2C, reg) ;
    if (readData[0] < 0) {
        last_status = errno;
        return 0;
    }

    readData[1] = i2c_smbus_read_byte(fI2C) ;
    if (readData[1] < 0) {
        last_status = errno;
        return 0;
    }
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
    ...
    readData[2] = i2c_smbus_read_byte(fI2C) ;
    if (readData[2] < 0) {
        last_status = errno;
        return 0;
    }

    readData[3] = i2c_smbus_read_byte(fI2C) ;
    if (readData[3] < 0) {
        last_status = errno;
        return 0;
    }
    ...
    value |=           readData[3];       // value lowest byte
    return value;
}
...
{
    int err;
    err = i2c_smbus_write_i2c_block_data(fI2C, reg, count, src) ;
    if (err < 0) {
        last_status = errno;
    }
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
    ...
    dst[0] = i2c_smbus_read_byte_data(fI2C, reg) ;
    if (dst[0] < 0) {
        last_status = errno;
        return;
    }
    ...
    for (i = 1; i < count; i++) {
        dst[i] = i2c_smbus_read_byte(fI2C) ;
        if (dst[i] < 0) {
            last_status = errno;
            return;
        }
    ...
    err = i2c_smbus_read_i2c_block_data(fI2C, reg, count, dst);  // see documentation at https://www.kernel.org/doc/Documentation/i2c/smbus-protocol
    if (err < 0) {
        last_status = errno;
        return;
    }
